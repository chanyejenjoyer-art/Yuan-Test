...
export class Game {
	...
	private resetWorldBounds() {
		...
		const halfW = W / 2;
		this.ground = Bodies.rectangle(halfW, H + T/2 - 2, W, T, { isStatic: true, friction: 0.1 });
		this.leftWall = Bodies.rectangle(-T/2 + 2, H/2, T, H, { isStatic: true });
		this.rightWall = Bodies.rectangle(W + T/2 - 2, H/2, T, H, { isStatic: true });
		this.ceiling = Bodies.rectangle(halfW, -T, W, T, { isStatic: true, isSensor: true });
